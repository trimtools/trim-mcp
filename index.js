...
server.tool(
  ...
  async ({ token }) => {
    try {
      ...
      const decode = (str) => JSON.parse(Buffer.from(str.replace(/-/g, "+").replace(/_/g, "/"), "base64").toString("utf-8"));
...
server.tool(
  ...
  async ({ type, count }) => {
    ...
    const randomWords = (n) => Array.from({ length: n }, () => words[Math.floor(Math.random() * words.length)]).join(" ");
